...
def getMissedEvents(request):
    ...
    assert isinstance (user, dict), f"In {getMissedEvents.cls.__name__}: expected user to be of type dictionary, instead got: {type(user)}"
    ...
    assert isinstance (projects, list), f"In {getMissedEvents.cls.__name__}: expected projects to be of type list, instead got: {type(projects)}"
