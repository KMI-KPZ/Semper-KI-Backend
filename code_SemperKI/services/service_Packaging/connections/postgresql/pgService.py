...
def initializeService(serviceDetails:dict) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in initializeService(Packaging): {str(error)}')
...
def updateServiceDetails(existingContent, newContent) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in updateServiceDetails(Packaging): {str(error)}')
...
def isFileRelevantForService(existingContent:dict, fileID:str) -> bool:
    ...
    try:
        ...
        logger.error(f'Generic error in isFileRelevantForService(Packaging): {str(error)}')
        return False
...
def serviceReady(existingContent:dict) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        logger.error(f'Generic error in serviceReady(Packaging): {str(error)}')
        return False
...
def cloneServiceDetails(existingContent:dict, newProcess:Process|ProcessInterface) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in cloneServiceDetails(Packaging): {str(error)}')
