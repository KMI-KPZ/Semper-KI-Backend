...
Contains: Signals send by the other apps which relate to the Create Model service
"""

import Generic_Backend.code_General.utilities.signals as GeneralSignals

from ..service import SERVICE_NUMBER
from ..connections.postgresql.pgProfilesSKICM import updateOrgaDetailsSemperKICM, deleteOrgaDetailsSemperKICM

################################################################################################

class CreateModelSignalReceivers():
    """
    ...
    def receiverForOrgaServiceSelection(sender, **kwargs):
        ...
        for service in selectedServices:
            if service == SERVICE_NUMBER:
                updateOrgaDetailsSemperKICM(orgaID)
    ...
    def receiverForOrgaServiceDeletion(sender, **kwargs):
        ...
        for service in selectedServices:
            if service == SERVICE_NUMBER:
                deleteOrgaDetailsSemperKICM(orgaID)
...
createModelSignalReceiver = CreateModelSignalReceivers()
