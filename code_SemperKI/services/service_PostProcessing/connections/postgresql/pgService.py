...
def initializeService(serviceDetails:dict) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in initializeService(Post Processing): {str(error)}')
...
def updateServiceDetails(existingContent, newContent) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in updateServiceDetails(Post Processing): {str(error)}')
...
def isFileRelevantForService(existingContent:dict, fileID:str) -> bool:
    ...
    try:
        ...
        logger.error(f'Generic error in isFileRelevantForService(Post Processing): {str(error)}')
        return False
...
def serviceReady(existingContent:dict) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        logger.error(f'Generic error in serviceReady(Post Processing): {str(error)}')
        return False
...
def cloneServiceDetails(existingContent:dict, newProcess:Process|ProcessInterface) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in cloneServiceDetails(Post Processing): {str(error)}')
