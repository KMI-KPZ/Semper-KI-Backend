...
def initializeService(serviceDetails:dict) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in initializeService(After Sales): {str(error)}')
...
def isFileRelevantForService(existingContent:dict, fileID:str) -> bool:
    ...
    try:
        ...
        logger.error(f'Generic error in isFileRelevantForService(After Sales): {str(error)}')
        return False
...
def serviceReady(existingContent:dict) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        logger.error(f'Generic error in serviceReady(After Sales): {str(error)}')
        return False
...
def cloneServiceDetails(existingContent:dict, newProcess:Process|ProcessInterface) -> dict:
    ...
    try:
        ...
        logger.error(f'Generic error in cloneServiceDetails(After Sales): {str(error)}')
